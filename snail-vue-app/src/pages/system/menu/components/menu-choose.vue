...
import { computed, h } from 'vue' // 添加缺少的导入
...
const task = computed(() => props.menu)
const title = computed(() => task.value?.id ? t('system.menu.form.title.edit') : t('system.menu.form.title.create'))
const description = computed(() => task.value?.id ? t('system.menu.form.title.editDescription') + task.value.id : t('system.menu.form.title.createDescription'))
...
const initialValues = computed(() => {
  if (task.value) {
    return {
      ...
      url: task.value.url || '',
      perms: task.value.perms || '',
      type: task.value.type,
      icon: task.value.icon || '',
      orderNum: task.value.orderNum ?? 0,
      description: task.value.description || '',
      parentId: task.value.parentId ?? 0,
    }
  }
  return {}
})
